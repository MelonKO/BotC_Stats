-- Games by alignment for players who played more than 1/3 of games in the period
WITH
period_games AS (
...
SELECT
    ...
    ROUND(COUNT(*) FILTER (WHERE gp.alignment_end = 'good')::numeric
        / NULLIF(COUNT(*), 0) * 100, 2)                                AS rate_good,
    ROUND(COUNT(*) FILTER (WHERE gp.alignment_end = 'evil')::numeric
        / NULLIF(COUNT(*), 0) * 100, 2)                                AS rate_evil
FROM game_players gp
JOIN games g   ON g.id = gp.game_id
